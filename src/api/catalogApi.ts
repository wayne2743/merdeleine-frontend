...
function normalizeProductSellWindowCombined(data: unknown): ProductSellWindowView {
  ...
  return {
    ...
    minQty: pickNumber(
      productSellWindow.minQty,
      productSellWindow.min_qty,
      productSellWindow.thresholdQty,
      productSellWindow.threshold_qty,
      quota.minQty,
      quota.min_qty,
      quota.thresholdQty,
      quota.threshold_qty,
    ),
    maxQty: pickNullableNumber(
      productSellWindow.maxQty,
      productSellWindow.max_qty,
      productSellWindow.maxTotalQty,
      productSellWindow.max_total_qty,
      quota.maxQty,
      quota.max_qty,
      quota.maxTotalQty,
      quota.max_total_qty,
...
function normalizeProduct(data: unknown): Product {
  ...
  return {
    ...
    ingredients: pickNullableString(raw.ingredients),
    allergens: pickNullableString(raw.allergens, raw.allergies),
    allergies: pickNullableString(raw.allergies, raw.allergens),
    calories: pickNullableNumber(raw.calories, raw.calorie),
    calorie: pickNullableNumber(raw.calorie, raw.calories),
...
export const catalogApi = {
  ...
  async createIngredientGroup(req: IngredientGroupCreateRequest): Promise<IngredientGroup> {
    const { data } = await http.post<IngredientGroup>(`/api/catalog/ingredient-groups`, req);
    return data;
  },

  async getIngredientGroup(id: string): Promise<IngredientGroup> {
    const { data } = await http.get<IngredientGroup>(`/api/catalog/ingredient-groups/${id}`);
    return data;
  },

  async listProductIngredientGroups(productId: string): Promise<IngredientGroup[]> {
    const { data } = await http.get<IngredientGroup[]>(`/api/catalog/products/${productId}/ingredient-groups`);
  ...
  async deleteIngredientGroup(id: string): Promise<void> {
    await http.delete(`/api/catalog/ingredient-groups/${id}`);
  },
